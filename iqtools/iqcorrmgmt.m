...
% Last Modified by GUIDE v2.5 13-Jun-2015 21:20:54
...
try
    ...
    axes(handles.axes1);
    if (~showPhase)
        plot(ampCorr(:,1), -1*ampCorr(:,2), 'LineWidth', 2);
        legend({'Magnitude'});
        ylabel('Loss (dB)');
    elseif (~showMag)
        plot(ampCorr(:,1), phase, 'LineWidth', 2);
        legend({'Phase'});
    ...
    if (~showPhase)
        plot(perChannelCorr(:,1), y1, 'Linewidth', 2);
        ...
        ylabel('Loss (dB)');
    elseif (~showMag)
        plot(perChannelCorr(:,1), y2, 'Linewidth', 2);
        switch (size(perChannelCorr,2))
            case 2; legend({'Phase'});
        ...
        case 1 % *.mat
            try
                ...
                elseif (~isfield(eq, 'Y') || ~isfield(eq, 'XStart') || ~isfield(eq, 'XDelta')) % VSA trace file
                    ...
                    if (~isempty(result))
                        freq = freq + eval(result{1});
                        updatePerChannelCorr(handles, freq, eq.Y);
                    end
                end
            catch ex
                errordlg({ex.message, [ex.stack(1).name ', line ' num2str(ex.stack(1).line)]});
